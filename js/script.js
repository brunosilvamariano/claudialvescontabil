...
document.addEventListener('DOMContentLoaded', function () {
 ...
 const form            = document.getElementById('tax-comparator-form');
  ...
  // Alerta em tempo real de elegibilidade (Limite mensal R$ 400.000)
  revenueInput?.addEventListener('input', function () {
  ...
  /* ── Simples Nacional — tabelas progressivas 2025/2026 ── */
  // Faixas e Deduções mantidas conforme LC 123/2006 (sem alteração legislativa de faixas até 2026)
  const SIMPLES = {
  ...
  const ANNEX_LABEL = { I:'Comércio – Anexo I', II:'Indústria – Anexo II', III:'Serviços – Anexo III', V:'Serviços – Anexo V' };
  ...
  function calcLP(revenue, activity) {
    ...
    // IRPJ: 15% sobre base presumida + 10% adicional sobre o que exceder R$ 20k/mês na base
    const baseIRPJ = revenue * presumIRPJ;
    const irpj = (baseIRPJ * 0.15) + (Math.max(0, baseIRPJ - 20000) * 0.10);
    
    const csll = (revenue * presumCSLL) * 0.09;
    const pisCofins = revenue * 0.0365; // Cumulativo
    const iss = isServ ? revenue * 0.03 : 0; // Média 3%
    
    const total = irpj + csll + pisCofins + iss;
    return {
      totalTax: total,
      rate: total / revenue,
      note: `Presunção ${(presumIRPJ*100).toFixed(0)}% p/ IRPJ + CSLL + PIS/COFINS (3,65%)${isServ ? ' + ISS (3%)' : ''}`
    };
  }

  /* ── Lucro Real (Regras 2026) ── */
  function calcLR(revenue, activity, profitMargin) {
    ...
    // PIS/COFINS Não-cumulativo (9,25%) - Estimativa de crédito de 50% médio
    ...
    return {
      ...
      note: `IRPJ/CSLL sobre lucro real (${(profitMargin*100).toFixed(0)}%) + PIS/COFINS não-cumulativo líquido${isServ ? ' + ISS' : ''}`
    };
  }
  ...
  function renderHeader(best) {
    ...
    const reasons = {
      simples: 'Regime simplificado com guia única (DAS). Geralmente vantajoso para empresas com faturamento até R$ 4,8M e folha de pagamento relevante.',
      presumido: 'Vantajoso para empresas com margens de lucro superiores à presunção (32% para serviços) e faturamento estável.',
      real: 'Indicado para empresas com margens de lucro baixas ou prejuízo operacional, permitindo tributar apenas o lucro efetivo.',
    };
  ...
  function renderSavings(eligible) {
    ...
    if (savingsYearly) savingsYearly.textContent = `Equivalente a R$ ${fmt(yearly)} ao ano — economia direta para o seu caixa.`;
  }
  ...
  function updateWA(best, revenue, activity) {
    ...
    const msg = encodeURIComponent(
      `Olá! Fiz a simulação na Calculadora Tributária 2026.\n\n` +
      ...
      `Gostaria de uma análise detalhada para minha empresa.`
    );
